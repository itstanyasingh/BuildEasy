  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 md:p-6 overflow-hidden">
      <div className="bg-white w-full max-w-7xl rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[94vh] border border-zinc-200">
        ...
        {/* Top Control Bar */}
        <div className="px-4 py-3.5 sm:px-6 border-b border-zinc-200 flex flex-wrap items-center justify-between gap-4 bg-zinc-50/90">
          
          {/* Left: Template info */}
          <div className="flex items-center gap-3">
            <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-zinc-200/80 text-zinc-800 font-mono">
              {template.category}
            </span>
            <div className="h-4 w-px bg-zinc-300 hidden sm:block"></div>
            <h2 className="text-lg sm:text-xl font-serif font-bold text-zinc-900 flex items-center gap-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-zinc-900 text-white font-medium text-xs flex items-center gap-2 hover:bg-zinc-800 transition-all shadow-sm cursor-pointer"
            >
            ...
            <button 
              onClick={onClose}
              className="p-2 rounded-xl text-zinc-400 hover:text-zinc-900 hover:bg-zinc-200/60 transition-colors cursor-pointer"
              title="Close Preview"
            >
              <X className="w-5 h-5" />
            </button>
        ...
        <div className="flex-1 bg-zinc-100 p-2 sm:p-6 overflow-hidden flex items-center justify-center relative">
          <div className={`transition-all duration-300 bg-white overflow-hidden shadow-2xl flex flex-col ${
            deviceMode === 'mobile' 
              ? 'w-[390px] h-[780px] max-h-[85vh] rounded-[44px] border-[10px] border-zinc-900 ring-1 ring-zinc-700/50' 
